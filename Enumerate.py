...
class Enumerate:
    ...
    def enumerate_library_strux(
            self, libname, rxschemefile, infilelist, outpath, rndct=-1, bblistfile=None, SMILEScolnames=[],
            BBIDcolnames=[],
        ...
        def library_partition(args, shard_file, prtn_indx, chunk_step, rxschemefile, libname, rndct, bbid, bbsmiles):
            ...
            if NUM_WORKERS > 1:
                ...
                data = list(
                    zip(chunked_args, rxschemefile, libname, rndct, schemeinfo, shard_file_chunk_names, bbid, bbsmiles))
                ...
                library_pipeline(
                    (args, rxschemefile, libname, rndct, schemeinfo, shard_file_chunk_name, bbid, bbsmiles))
    ...
    def EnumFromBBFiles(
        ...
        outfile = self.enumerate_library_strux(
            ...
            SMILEScolnames=SMILEScolnames, BBIDcolnames=BBcolnames, removeduplicateproducts=rem_dups,
            write_fails_enums=write_fails_enums,
...
class EnumerationCLI:
    ...
    def Run_CLI():
        ...
        paramdefaults = [('rxnschemefile', './RxnSchemes.json'), ('schemepath', '.'), ('scheme', ''),
                         ('schemespec', ''), ('numstrux', 5000), ('removedups', False)]
        ...
        outf = enum.EnumFromBBFiles(
            args['scheme'], args['schemespec'], args['schemespec'], args['schemepath'], args['scheme'] + addspec,
            args['numstrux'], args['rxnschemefile'],
            rem_dups=rd, write_fails_enums=write_fails_enums, prtn_indx=args["partitionindex"],
            prtn_step=args["partitionstep"], chunk_step=args["chunkstep"],
